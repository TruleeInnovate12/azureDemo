...
const UserForm = ({ isOpen, onClose, onDataAdded }) => {
  ...
  const handleGenderSelect = (gender) => {
    ...
    setUserData((prevUserData) => ({
      ...prevUserData,
      Gender: gender, // Update Gender in userData
    }));
    setShowDropdownGender(false); // Close the dropdown
    setErrors((prevErrors) => ({
      ...prevErrors,
      Gender: "", // Clear the error for Gender
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Check if user with the same sub already exists
      const existingUserResponse = await axios.get(`${process.env.REACT_APP_API_URL}/users?sub=${dataToSend.sub}`);
      if (existingUserResponse.data) {
        console.error('User with this sub already exists');
        return;
      }
      ...
      if (contactResponse.data) {
        onDataAdded(contactResponse.data);
      }
  ...
  const handleTimezoneChange = (timezone) => {
    ...
    setUserData((prevUserData) => ({
      ...prevUserData,
      TimeZone: timezone.value, // Update TimeZone in userData
    }));
    setErrors((prevErrors) => ({
      ...prevErrors,
      TimeZone: "", // Clear the error for TimeZone
    }));
  };
  ...
  const handleLanguageSelect = (language) => {
    ...
    setUserData((prevUserData) => ({
      ...prevUserData,
      Language: language, // Update Language in userData
    }));
    setShowDropdownLanguage(false);
    setErrors((prevErrors) => ({
      ...prevErrors,
      Language: "", // Clear the error for Language
    }));
  };
  ...
  const handleClose = () => {
    ...
    const isFormEmpty = !userData.FirstName && !userData.LastName && !userData.Email && !userData.Phone && !userData.UserID && !userData.Password && !userData.LinkedinURL && !userData.TimeZone && !userData.Language && !userData.Gender && !selectedProfile && !selectedCurrentRole;

    if (!isFormEmpty) {
      setShowConfirmationPopup(true); // Show confirmation if form is not empty
    } else {
      onClose(); // Call onClose to close the form and return to Users.jsx
    }
  };

  const handleConfirmClose = () => {
    onClose();
    console.log("Confirmed close, closing form");
  };

  const handleCancelClose = () => {
    setShowConfirmationPopup(false);
    console.log("Cancelled close, keeping form open");
  };
                  ...
                  <div className="flex gap-5 mb-5">
                    ...
                    <div className="flex-grow">
                      <input
                        ...
                        onChange={handleChange}
                        autoComplete="off"
                        className={`border-b focus:outline-none mb-5 w-full ${errors.UserID ? "border-red-500" : "border-gray-300 focus:border-black"}`}
                      />
                      {errors.UserID && <p className="text-red-500 text-sm -mt-4">{errors.UserID}</p>}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-5 mb-9">
                    ...
                    <div className="relative flex-grow">
                      <input
                        ...
                        readOnly
                        className={`border-b focus:outline-none mb-5 w-full ${errors.ProfileId ? "border-red-500" : "border-gray-300 focus:border-black"}`}
                      />
                      {errors.ProfileId && <p className="text-red-500 text-sm -mt-4">{errors.ProfileId}</p>}
                      <div
                  ...
                  <div className="flex gap-5 mb-9">
                    ...
                    <div className="relative flex-grow">
                      <input
                        ...
                        type="text"
                        id="CurrentRole"
                        className={`border-b focus:outline-none mb-5 w-full ${errors.RoleId ? "border-red-500" : "border-gray-300 focus:border-black"}`}
                        readOnly
                        onClick={toggleDropdownRole}
                        value={selectedCurrentRole || ""}
                      />
                      {errors.RoleId && <p className="text-red-500 text-sm -mt-4">{errors.RoleId}</p>}
                      <div
                        className="absolute right-0 top-0 mt-1 cursor-pointer"
                        onClick={toggleDropdownRole}
                      >
                    ...
                    <div className="flex-grow w-full overflow-visible -mt-1 relative">
                      <div className="w-full overflow-visible">
                        <input
                          ...
                          type="text"
                          id="TimeZone"
                          className={`border-b focus:outline-none mb-5 w-full ${errors.TimeZone ? "border-red-500" : "border-gray-300 focus:border-black"}`}
                          readOnly
                          onClick={toggleDropdownTimezone}
                          value={selectedTimezone.label || ""}
                        />
                        {errors.TimeZone && <p className="text-red-500 text-sm -mt-4">{errors.TimeZone}</p>}
                        <div
                          className="absolute right-0 top-0 mt-1 cursor-pointer"
                          onClick={toggleDropdownTimezone}
                        >
                      ...
                      <div className="relative flex-grow">
                        <div className="relative">
                          <input
                            type="text"
                            id="language"
                            className={`border-b focus:outline-none mb-5 w-full ${errors.Language ? "border-red-500" : "border-gray-300 focus:border-black"}`}
                            readOnly
                            onClick={toggleDropdownLanguage}
                            value={selectedLanguage || ""}
                          />
                          {errors.Language && <p className="text-red-500 text-sm -mt-4">{errors.Language}</p>}
                          <div
                            className="absolute right-0 top-0 mt-1 cursor-pointer"
                            onClick={toggleDropdownLanguage}
                          >
